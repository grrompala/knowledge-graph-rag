...
def train(
    ...
    qa_dataset = pd.read_csv("stark_qa_final_enriched_starkmini.tsv",sep="\t")
    train_indices = load_indices("splits/train.index")
    val_indices = load_indices("splits/val.index")
    test_indics = load_indices("splits/test.index")
    ...
    qa_raw_train = SimpleQADataset(qa_dataset, train_indices)
    #qa_raw_train = qa_dataset.get_subset('train')
    qa_raw_val = SimpleQADataset(qa_dataset, val_indices)
    #qa_raw_val = qa_dataset.get_subset('val')
    qa_raw_test = SimpleQADataset(qa_dataset, test_indices)
